...
test.describe('Data Persistence', () => {
  ...
  test('learning session restores after app restart', async () => {
    ...
    const spanishSession = session2.sessions
      ? Object.values(session2.sessions).find(
          (s: any) => s.learningProgress && s.learningProgress.currentWordIndex === 1
        )
      : null;

    // If not found by word index, try to find any Spanish session
    const spanishSessionAlt = session2.sessions
      ? Object.values(session2.sessions).find(
          (s: any) => s.learningProgress && s.learningProgress.currentWordIndex !== undefined
        )
      : null;
