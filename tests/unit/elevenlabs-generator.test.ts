...
describe('ElevenLabs Audio Generator', () => {
  ...
  describe('generateAudio', () => {
    it('should throw error when API key is not configured', async () => {
      ...
      await expect(generatorWithoutKey.generateAudio('hello', 'spanish')).rejects.toThrow(
        'ElevenLabs API key not configured'
      );
    });

    it('should validate input parameters', async () => {
      await expect(generator.generateAudio('', 'spanish')).rejects.toThrow('Text cannot be empty');

      await expect(generator.generateAudio('   ', 'spanish')).rejects.toThrow(
