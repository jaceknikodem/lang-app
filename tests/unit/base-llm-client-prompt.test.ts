...
class TestLLMClient extends BaseLLMClient {
  ...
  public testCreateTopicWordsPrompt(topic: string, language: string, count: number, existingWords: string[] = [], proficiencyLevel?: string): string {
    return this.createTopicWordsPrompt(topic, language, count, existingWords, proficiencyLevel);
  }

  public testCreateSentencesPrompt(word: string, language: string, count: number, knownWords: string[] = [], topic?: string, proficiencyLevel?: string): string {
    return this.createSentencesPrompt(word, language, count, knownWords, topic, proficiencyLevel);
  }

  public testCreateContextSentencesPrompt(sentence: string, translation: string, language: string): string {
    return this.createContextSentencesPrompt(sentence, translation, language);
  }

  public testCreateDialogueVariantPrompt(triggerSentence: string, triggerTranslation: string, language: string, knownWords: string[], count: number): string {
...
describe('BaseLLMClient Prompt Generation', () => {
  ...
  describe('createSentencesPrompt', () => {
    it('should generate prompt with basic parameters', () => {
      ...
      expect(prompt).toContain('Generate exactly 3 natural, conversational sentences in Spanish using the word \'hola\'');
    ...
    it('should include topic guidance when provided', () => {
      ...
      expect(prompt).toContain('All sentences should relate to or be contextually relevant to the topic: "food"');
    });

    it('should not include topic text when not provided', () => {
      const prompt = client.testCreateSentencesPrompt('hola', 'Spanish', 3);
      
      expect(prompt).not.toContain('All sentences should relate to or be contextually relevant to the topic');
    });

    it('should include proficiency level guidance for newbie', () => {
      const prompt = client.testCreateSentencesPrompt('hola', 'Spanish', 3, [], undefined, 'newbie');
      
      expect(prompt).toContain("user's proficiency level is NEWBIE");
      expect(prompt).toContain('Use very simple sentence structures, basic grammar, and common words');
    });
    ...
    it('should include proficiency level guidance for a2', () => {
      ...
      expect(prompt).toContain("user's proficiency level is A2");
      expect(prompt).toContain('Use common sentence structures appropriate for A2 elementary learners');
    });
  ...
  describe('createContextSentencesPrompt', () => {
    it('should generate prompt with sentence and translation', () => {
      const prompt = client.testCreateContextSentencesPrompt('Hola, ¿cómo estás?', 'Hello, how are you?', 'Spanish');
