...
class MockOllamaClient {
    ...
    generateTopicWords = jest.fn().mockImplementation((topic: string, language: string, count: number, proficiencyLevel?: string): Promise<GeneratedWord[]> => {
    ...
    generateSentences = jest.fn().mockImplementation((word: string, language: string, count: number, topic?: string, proficiencyLevel?: string): Promise<GeneratedSentence[]> => {
...
describe('ContentGenerator', () => {
    ...
    describe('generateWordSentences', () => {
        ...
        beforeEach(() => {
            ...
            mockClient.generateSentences.mockImplementation((word: string, language: string, count: number) => {
                const sentences: GeneratedSentence[] = [];
                for (let i = 1; i <= count; i++) {
                    sentences.push({
                        sentence: `${word} sentence ${i}`,
        ...
        it('should generate sentences using LLM when Tatoeba is unavailable', async () => {
            ...
            const sentences = await contentGenerator.generateWordSentences('hola', 'Spanish', 3, mockDatabase as DatabaseLayer);
        ...
        it('should handle empty word parameter', async () => {
            await expect(contentGenerator.generateWordSentences('', 'Spanish', 3)).rejects.toThrow('Word parameter is required');
        });
        ...
        it('should handle topic parameter', async () => {
            ...
            expect(mockClient.generateSentences).toHaveBeenCalledWith('hola', 'Spanish', expect.any(Number), 'greetings', undefined);
        });

        it('should handle proficiency level from database', async () => {
            mockDatabase.getSetting = jest.fn().mockResolvedValue('A1');
            await contentGenerator.generateWordSentences('hola', 'Spanish', 3, mockDatabase as DatabaseLayer);
        ...
        it('should throw error when no valid sentences generated', async () => {
            ...
            try {
                ...
                // Error is wrapped, check cause
                expect(error.cause?.message || error.message).toContain('No valid sentences were generated');
            }
        });
    });
    ...
    describe('frequency word management', () => {
        ...
        describe('getFrequencyProgress', () => {
            it('should return frequency progress', async () => {
                ...
                const progress = await contentGenerator.getFrequencyProgress('spanish', mockDatabase as DatabaseLayer);

                expect(progress).toEqual(mockProgress);
                expect(mockFrequencyWordManager.getLanguageProgress).toHaveBeenCalledWith('spanish', mockDatabase);
            });
        });
        ...
        describe('generateFrequencyVocabulary', () => {
            it('should generate frequency-based vocabulary', async () => {
                ...
                mockFrequencyWordManager.getNextWordsToProcess = jest.fn().mockResolvedValue(mockWordEntries);
                mockClient.generateResponse.mockResolvedValue('translation');

                const words = await (contentGenerator as any).generateFrequencyBasedVocabulary('spanish', 2, mockDatabase as DatabaseLayer);
            ...
            it('should throw error when no more words available', async () => {
                ...
                await expect((contentGenerator as any).generateFrequencyBasedVocabulary('spanish', 2, mockDatabase as DatabaseLayer))
            ...
            it('should filter words by proficiency level A1', async () => {
                ...
                expect(words.every((w: GeneratedWord) => !w.frequencyPosition || w.frequencyPosition > 200)).toBe(true);
            });
            ...
            it('should filter words by proficiency level A2', async () => {
                ...
                expect(words.every((w: GeneratedWord) => !w.frequencyPosition || w.frequencyPosition > 500)).toBe(true);
            });
            ...
            it('should filter words by proficiency level B1', async () => {
                ...
                expect(words.every((w: GeneratedWord) => !w.frequencyPosition || w.frequencyPosition > 1000)).toBe(true);
            });
    ...
    describe('helper methods', () => {
        ...
        describe('filterWordsByProficiencyLevel', () => {
            it('should filter A1 words (top 200)', () => {
                ...
                const filtered = (contentGenerator as any).filterWordsByProficiencyLevel(words, 'A1', 'spanish');
            ...
            it('should filter A2 words (top 500)', () => {
                ...
                const filtered = (contentGenerator as any).filterWordsByProficiencyLevel(words, 'A2', 'spanish');
            ...
            it('should filter B1 words (top 1000)', () => {
                ...
                const filtered = (contentGenerator as any).filterWordsByProficiencyLevel(words, 'B1', 'spanish');
            ...
            it('should not filter for other levels', () => {
                ...
                const filtered = (contentGenerator as any).filterWordsByProficiencyLevel(words, 'B2', 'spanish');
        ...
        describe('getWordTranslation', () => {
            ...
            it('should throw error when LLM is unavailable', async () => {
                ...
                await expect((contentGenerator as any).getWordTranslation('hola', 'Spanish'))
        ...
        describe('fetchTatoebaExamples', () => {
            it('should return empty array for unsupported language', async () => {
                const result = await (contentGenerator as any).fetchTatoebaExamples('word', 'unsupported', 3);
