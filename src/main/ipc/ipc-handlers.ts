...
function setupDialogHandlers(
  ...
  registerHandlers([
    ...
    {
      channel: IPC_CHANNELS.DIALOG.ENSURE_BEFORE_SENTENCE_AUDIO,
      schema: SentenceIdSchema,
      description: 'ensure before sentence audio',
      handler: async (sentenceId) => {
        const sentence = await databaseLayer.getSentenceById(sentenceId);
        if (!sentence) throw new Error(`Sentence with ID ${sentenceId} not found`);
        if (!sentence.contextBefore) return null;
        if (sentence.beforeSentenceAudioPath) return sentence.beforeSentenceAudioPath;
    ...
    {
      channel: IPC_CHANNELS.DIALOG.ENSURE_CONTEXT_SENTENCES,
      schema: SentenceIdSchema,
      description: 'ensure context sentences audio',
      handler: async (sentenceId) => {
        const sentence = await databaseLayer.getSentenceById(sentenceId);
        if (!sentence) throw new Error(`Sentence with ID ${sentenceId} not found`);
        ...
        let contextVoiceId: string | undefined;
        ...
        return { beforeSentenceAudio, afterSentenceAudio };
      },
    },
  ]);
}
