...
export const GeneratedWordSchema = z.object({
  word: z.string().min(1, "Word cannot be empty").transform(cleanSpecialTokens).pipe(z.string().min(1)),
  translation: z.string().min(1, "Translation cannot be empty").transform(cleanSpecialTokens).pipe(z.string().min(1))
});

export const GeneratedSentenceSchema = z.object({
  sentence: z.string().min(1, "Sentence cannot be empty").transform(cleanSpecialTokens).pipe(z.string().min(1)),
  translation: z.string().min(1, "Translation cannot be empty").transform(cleanSpecialTokens).pipe(z.string().min(1)),
  contextBefore: z.string().optional().transform(s => s ? cleanSpecialTokens(s) : undefined),
  contextAfter: z.string().optional().transform(s => s ? cleanSpecialTokens(s) : undefined),
  contextBeforeTranslation: z.string().optional().transform(s => s ? cleanSpecialTokens(s) : undefined),
  contextAfterTranslation: z.string().optional().transform(s => s ? cleanSpecialTokens(s) : undefined)
});

// Fallback schemas for when LLM returns unexpected formats
export const LooseWordSchema = z.object({
  word: z.string().transform(s => cleanSpecialTokens(s)).pipe(z.string().min(1)),
  translation: z.string().transform(s => cleanSpecialTokens(s)).pipe(z.string().min(1))
}).transform(obj => ({
  word: obj.word,
...
export const LooseSentenceSchema = z.object({
  sentence: z.string().transform(s => cleanSpecialTokens(s)).pipe(z.string().min(1)),
  translation: z.string().transform(s => cleanSpecialTokens(s)).pipe(z.string().min(1)),
  contextBefore: z.string().optional().transform(s => s ? cleanSpecialTokens(s) : undefined),
  contextAfter: z.string().optional().transform(s => s ? cleanSpecialTokens(s) : undefined),
  contextBeforeTranslation: z.string().optional().transform(s => s ? cleanSpecialTokens(s) : undefined),
...
export const ContextSentenceResponseSchema = z.union([
  ...
  z.record(z.any()).transform(obj => ({
    ...
    contextBeforeTranslation: obj.contextBeforeTranslation ? cleanSpecialTokens(String(obj.contextBeforeTranslation)) : undefined,
    contextAfterTranslation: obj.contextAfterTranslation ? cleanSpecialTokens(String(obj.contextAfterTranslation)) : undefined
  }))
]);
...
export const FollowUpResponseSchema = z.union([
  ...
  z.union([
    ...
    z.record(z.any()).transform(obj => {
      ...
      const translation = cleanSpecialTokens(String((obj as any).translation || (obj as any).english || ''));
